...
Ext.extend(Ext.ux.PasswordMeter, Ext.form.TextField, {
		...
		calcStrength: function(p) {
			...
			if (p.length > 0 && p.length <= 3) {                    // length 4 or less
				intScore += 2;
			}
			else if (p.length >= 4 && p.length <= 6) {	// length between 5 and 7
				intScore += 6;
			}
			else if (p.length >= 7 && p.length <= 10) {	// length between 8 and 15
				intScore += 15;
				//alert(intScore);
			}
			else if (p.length >= 11) {               // length 16 or more
				intScore += 20;
				//alert(intScore);
			}
