Ext.ux.grid.filter.DateFilter = Ext.extend(Ext.ux.grid.filter.Filter, {
	...
	serialize: function() {
		...
		if(this.dates.before.checked) {
			args = [{type: 'date', comparison: '<', value: "'"+ this.getFieldValue('before').format(this.dateFormat)+"'"}];
    }
		if(this.dates.after.checked) {
			args.push({type: 'date', comparison: '>', value: "'"+ this.getFieldValue('after').format(this.dateFormat)+"'"});
    }
		if(this.dates.on.checked) {
			args = {type: 'date', comparison: '=', value:"'"+ this.getFieldValue('on').format(this.dateFormat)+"'"};
    }
