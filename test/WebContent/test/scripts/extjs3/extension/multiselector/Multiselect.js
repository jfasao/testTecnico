...
Ext.ux.Multiselect = Ext.extend(Ext.form.Field,  {
    ...
    onRender: function(ct, position){
		...
		fs = new Ext.form.FieldSet({
			...
			height:this.height,
			width:this.width,			
			style:"padding:1px;border:1px;background-color: #c6c6c6;",
			tbar:this.tbar
		});
...
Ext.ux.ItemSelector = Ext.extend(Ext.form.Field,  {
	...
	iconRightAll:'rightall2.gif',
	iconLeftAll:'leftall2.gif',
	showDrawAll:false,
	defaultOrderBy:true,
	drawUpIcon:true,
	drawDownIcon:true,
	...
	getFromValues: function(fieldName){
		var cmp = Ext.getCmp(this.id).fromMultiselect;
		var c = Ext.getCmp(this.id).fromMultiselect.store.getCount();
		var result = new Array();
		for (i=0;i<c;i++){
			result.push(cmp.store.getAt(i).get(fieldName));
		}
		return result;
	},
	getToValues: function(fieldName){
		var cmp = Ext.getCmp(this.id).toMultiselect;
		var c = Ext.getCmp(this.id).toMultiselect.store.getCount();
		var result = new Array();
		for (i=0;i<c;i++){
			result.push(cmp.store.getAt(i).get(fieldName));
		}
    ...
    onRender: function(ct, position){
		...
		{
			this.iconUp = this.imagePath + (this.iconRightAll || 'rightall2.gif');
			this.iconDown = this.imagePath + (this.iconLeftAll || 'leftall2.gif');
		}
		...
		if (!this.readOnly) {
			if (!this.toSortField) {
				...
				if (this.showDrawAll){
					this.upIcon.on('click', this.rightAll, this);
					this.downIcon.on('click', this.leftAll, this);	
				}
	...
	rightAll:function(){this.allTo(!this.defaultOrderBy);},
	leftAll:function(){this.reset(!this.defaultOrderBy);},
	...
	reset: function(field){
		...
		if (!this.allowDup) {
			...
			this.fromMultiselect.store.sort((field)?this.valueField:this.displayField,'ASC');
		}
		this.valueChanged(this.toMultiselect.store);
	},
	
	allTo: function(field){
		range = this.fromMultiselect.store.getRange();
		this.fromMultiselect.store.removeAll();
		if (!this.allowDup) {
			this.toMultiselect.store.add(range);
			this.toMultiselect.store.sort((field)?this.valueField:this.displayField,'ASC');
		}
